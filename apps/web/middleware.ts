...
const PUBLIC_ASSETS=["/rebattery-favicon.svg","/rebattery-workspace-icon.svg"];
...
export async function middleware(req:NextRequest){const p=req.nextUrl.pathname;if(PUBLIC_ASSETS.includes(p)||PUBLIC.some(x=>p===x||p.startsWith(x)))return NextResponse.next(); const token=req.cookies.get("denchclaw_session")?.value; if(!token||!(await validateSessionToken(token))){if(p.startsWith("/api/"))return NextResponse.json({error:"Unauthorized"},{status:401}); const u=req.nextUrl.clone();u.pathname="/login";u.searchParams.set("next",p);return NextResponse.redirect(u);} if(req.method!=="GET"&&req.method!=="HEAD"&&req.method!=="OPTIONS"){const origin=req.headers.get("origin"); if(origin&&origin!==req.nextUrl.origin)return NextResponse.json({error:"CSRF check failed"},{status:403});} return NextResponse.next();}
