...
function makeClient() {
  txQuery = vi.fn(async (sql: string, params?: unknown[]) => {
    ...
    // Junction tables
    if (
      sql.includes("delete from crm_email_thread_participants") ||
      sql.includes("delete from crm_email_message_recipients") ||
      sql.includes("delete from crm_calendar_event_attendees")
    ) {
      return { rows: [] };
    }

    if (
      sql.includes("insert into crm_email_thread_participants") ||
      sql.includes("insert into crm_email_message_recipients") ||
      sql.includes("insert into crm_calendar_event_attendees")
    ) {
      return { rows: [] };
    }
...
describe("crm-postgres entry mutations", () => {
  ...
  it("creates canonical values in the entity table", async () => {
    ...
    const insert = queryCalls.find(
      ([sql]) =>
        String(sql).includes("insert into") &&
        String(sql).includes("crm_people"),
    );
  ...
  it("updates many_to_many relation through junction tables", async () => {
    ...
    const result = await updatePostgresEntry("people", "entry_1", {
      Company: ["c1", "c2"],
    });
    ...
    expect(queryCalls.some((sql) => sql.includes("crm_relation_links"))).toBe(
      false,
    );
  });
  ...
  it("deletes one entry with full cleanup and canonical row removal", async () => {
    ...
    expect(
      queryCalls.some((sql) => sql.includes("delete from crm_documents")),
    ).toBe(true);
    expect(
      queryCalls.some(
        (sql) =>
  ...
  it("bulk deletes entries and reports deleted count", async () => {
    ...
    expect(
      queryCalls.some(
        (sql) =>
          sql.includes("delete from crm_documents") &&
          sql.includes("entry_id = any($1::text[])"),
      ),
    ).toBe(true);
    expect(
      queryCalls.some(
  ...
  it("uses inferred canonical table when entity_table is null for people", async () => {
    ...
    const { createPostgresEntry, updatePostgresEntry, deletePostgresEntry } =
      await import("./entry-mutations");

    await createPostgresEntry("people", { "Full Name": "Ada" });
    await updatePostgresEntry("people", "entry_1", {
    ...
    expect(
      sqlCalls.some(
        (sql) => sql.includes("insert into") && sql.includes("crm_people"),
      ),
    ).toBe(true);
    expect(
      sqlCalls.some(
  ...
  it("rejects custom-only update when entry does not exist and writes no custom values", async () => {
    ...
    await expect(
      updatePostgresEntry("task", "missing_1", { Notes: "nope" }),
    ).rejects.toThrow("Entry not found: missing_1");
  });
  ...
  it("rejects mutations for loop monitoring projections", async () => {
    ...
    await expect(createPostgresEntry("automation_loop", {})).rejects.toThrow(
      "read-only",
    );
    await expect(
      updatePostgresEntry("automation_loop_run", "activity-1", {}),
    ).rejects.toThrow("read-only");
    await expect(
      deletePostgresEntry("automation_loop", "loop-1"),
    ).rejects.toThrow("read-only");
    await expect(
  ...
  it("updates canonical many_to_one relation field in canonical table only", async () => {
    ...
    const canonicalUpdate = calls.find(
      ([sql]) =>
        String(sql).includes("update") && String(sql).includes("crm_people"),
    );
    expect(canonicalUpdate?.[0]).toContain("company_id");
    expect(canonicalUpdate?.[1]).toEqual(["c9", "entry_1"]);
    expect(
      calls.some(([sql]) => String(sql).includes("crm_relation_links")),
    ).toBe(false);
  });
});
