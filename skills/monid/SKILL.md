...
**Use Monid after Composio.** For an external-service task, first search the official Composio CLI with `composio search "<task>"`. If Composio has a suitable tool, use it. Run `monid discover` only when Composio has no appropriate tool, Composio is unavailable, or the user explicitly asks for Monid. A missing Composio connection is not a missing tool: use `composio link <toolkit>` instead of falling back.

1. **Discover** — After the Composio check fails to find a suitable tool, run `monid discover -q "<what you need>"` to search available endpoints. Use `-s <score>` to filter by minimum relevance.
...
1. **Composio first, Monid second** — for external-service work, run `composio search` first. Use Monid only when Composio has no suitable tool, is unavailable, or the user explicitly requests Monid. If Composio finds a tool but its account is disconnected, run `composio link`; do not fall back merely to avoid authorization.
