...
6. If Composio has no suitable tool or the CLI is unavailable, load the [Monid skill](../monid/SKILL.md) and use its discover → inspect → run fallback. Do not fall back merely because a suitable Composio tool needs account authorization; run `composio link <toolkit>` instead.
